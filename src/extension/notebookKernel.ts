...
export class NotebookKernel {
    readonly id: string = 'select-book-kernel';
    readonly notebookType: string = 'select-book';
    readonly label: string = 'Select Book';
    readonly supportedLanguages = ['select-book'];
    ...
    private async _doExecution(cell: vscode.NotebookCell): Promise<void> {
        ...
        try {
            ...
            // we are already done here, the response is in data, add to interface now!
            try {
                const response = new ResponseParser({
                    ...
                    config: {
                        timeout: 0,
                        placeForMore: "nothing to report",
                        pathToProject: ""
//                        xsrfCookieName: "",
 //                       xsrfHeaderName: ""
                    }
                }, req, parser);
                updateCache(parser, response);
