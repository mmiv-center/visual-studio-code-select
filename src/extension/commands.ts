...
export function registerCommands(extensionId: string): vscode.Disposable {
	...
	subscriptions.push(vscode.commands.registerCommand('select-book.newNotebook', async () => {
		const newNotebook = await vscode.workspace.openNotebookDocument('select-book', 
			new vscode.NotebookData([
				new vscode.NotebookCellData(vscode.NotebookCellKind.Markup, '# Select\nA DICOM data exploration notebook build with ror.', 'markdown'),
				new vscode.NotebookCellData(vscode.NotebookCellKind.Code, 'SELECT series\n  FROM study\n    WHERE series NAMED "no-name" HAS\n        ("8","1090") containing LightSpeed16', 'select-book')
			]));
	...
	subscriptions.push(vscode.commands.registerCommand('select-book.newInteractive', async () => {
		const result: { inputUri: vscode.Uri, notebookUri?: vscode.Uri, notebookEditor?: vscode.NotebookEditor } | undefined = await vscode.commands.executeCommand('interactive.open',
			undefined,
			undefined,
			`${extensionId}/select-book-interactive-kernel`,
			undefined
		);
	}));
